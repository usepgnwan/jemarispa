...
export default function Edit({ employee, skills = [], certifications = [] }) {
    const { data, setData, post, processing, errors } = useForm({
        ...
        work_area: employee.work_area || 'Jemari Home Spa - Seluruh Area Layanan',
        ...
        certifications: employee.certifications && employee.certifications.length > 0
            ? employee.certifications.map(c => ({
                ...
                valid_until: c.pivot?.valid_until || '',
                existing_file: c.pivot?.certificate_file || null,
                file: null
              }))
            : [{ certification_id: '', certificate_number: '', valid_until: '', existing_file: null, file: null }]
    });
    ...
    const addCertificationRow = () => {
        setData('certifications', [...data.certifications, { certification_id: '', certificate_number: '', valid_until: '', existing_file: null, file: null }]);
    };
    ...
    const submit = (e) => {
        e.preventDefault();
        post(route('admin.employee.update', employee.id), { forceFormData: true });
    };
                                                        ...
                                                        <div className="flex items-center gap-2">
                                                            <label className="cursor-pointer inline-flex items-center gap-1.5 px-3 py-2 bg-white border border-gray-300 hover:border-[#0057B8] rounded-xl text-xs font-medium text-gray-700 transition-colors w-full justify-center">
                                                                ...
                                                                <span className="truncate max-w-[150px]">
                                                                    {row.file ? row.file.name : 'Ganti / Upload File'}
                                                                </span>
                                                                <input
                                                                    type="file"
                                                                    accept="image/*,.pdf"
                                                            ...
                                                            {row.existing_file && (
                                                                <a
                                                                    href={`/storage/${row.existing_file}`}
                                                                    target="_blank"
                                                                    rel="noreferrer"
                                                                    className="inline-flex items-center gap-1 px-2.5 py-2 bg-emerald-50 text-emerald-700 rounded-xl text-xs font-semibold hover:bg-emerald-100 transition-colors shrink-0"
                                                                    title="Lihat sertifikat tersimpan"
                                                                >
